...
class TestPushAdapter extends PushAdapter {
  async onConnect(_config: AdapterConfig): Promise<void> {}
  async onDisconnect(): Promise<void> {}

  simulateEvent(data: string): void {
    this.emit(data);
  }
}

class TestPollingAdapter extends PollingAdapter {
  private pollData: RawEvent[] = [];
  private offset = 0;

  setPollData(data: RawEvent[]): void {
    this.pollData = data;
  }

  protected async onConnect(_config: AdapterConfig): Promise<void> {}
  protected async onDisconnect(): Promise<void> {}

  protected async poll(_cursor?: CursorState): Promise<RawEvent[]> {
